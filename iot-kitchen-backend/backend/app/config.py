...
class Settings:
    ...
    ai_service_url: str = "http://ai:9100"
    ai_interval_s: float = 10.0        # chu kỳ suy luận, Bản thống nhất mục 6
    ai_window_size: int = 20           # sẽ được ghi đè theo mô hình thật qua /health
    ai_sample_seconds: int = 10        # chu kỳ 1 mẫu mà TV5 dùng lúc train (KHÔNG phải 2s của cảm biến)
...
def load_settings() -> Settings:
    return Settings(
        ...
        read_only=os.getenv("READ_ONLY", "false").strip().lower() in ("true", "1", "yes"),
        ai_enabled=os.getenv("AI_ENABLED", "true").strip().lower() not in ("false", "0", "no"),
        ai_service_url=os.getenv("AI_SERVICE_URL", "http://ai:9100").rstrip("/"),
        ai_interval_s=float(os.getenv("AI_INTERVAL_SECONDS", "10")),
