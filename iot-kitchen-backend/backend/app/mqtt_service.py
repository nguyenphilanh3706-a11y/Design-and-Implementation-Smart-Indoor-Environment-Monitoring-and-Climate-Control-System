...
class MqttService:
    ...
    async def _run(self) -> None:
        delay = 1
        while True:
            # TLS chỉ bật khi chạy trên Internet (broker đám mây như HiveMQ, hoặc Mosquitto
            # có chứng chỉ). Trong LAN thì để MQTT_TLS=false cho nhẹ.
            tls_params = aiomqtt.TLSParameters(ca_certs=self.s.mqtt_ca_cert or None) if self.s.mqtt_tls else None
