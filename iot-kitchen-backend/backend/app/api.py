...
#  POST /actuator
# =====================================================================
@router.post("/actuator", response_model=ActuatorResponse, dependencies=[Depends(require_write_access)], tags=["Điều khiển"],
...
#  POST /mode
# =====================================================================
@router.post("/mode", response_model=ModeResponse, dependencies=[Depends(require_write_access)], tags=["Điều khiển"],
...
@router.post("/config", response_model=ConfigResponse, dependencies=[Depends(require_write_access)], tags=["Cấu hình"],
...
async def get_prediction_accuracy(db: DbDep, settings: SettingsDep, device_id: DeviceQuery = None,
    ...
    Chỉ số MAE mà `train_ai.py` in ra được tính trên dữ liệu mô phỏng hình sin, nên nó cho
    biết mô hình học thuộc bài tốt đến đâu chứ không cho biết nó dự báo bếp thật tốt đến đâu.
    Con số ở đây mới là con số nên đưa vào báo cáo, và nên chạy sau khi hệ thống đã thu
    dữ liệu thật ít nhất vài giờ.
    """
    device = resolve_device(settings, device_id)
    r = await db.prediction_accuracy(device, hours)
    evaluated = int(r["so_mau_doi_chieu"] or 0)
    ratio = round(100 * int(r["doan_dung_xu_huong"] or 0) / evaluated, 1) if evaluated else None
    note = ("Chưa đủ dữ liệu: cần chờ ít nhất 15 phút sau lần dự báo đầu tiên để có giá trị thực đo "
            "mà đối chiếu." if evaluated == 0 else
            f"Tính trên {evaluated} dự báo đã tới hạn trong {hours} giờ gần nhất, "
            f"đối chiếu với giá trị đo thật quanh mốc dự báo (±15 giây).")
    rounded = lambda key: round(float(r[key]), 2) if r[key] is not None else None   # noqa: E731
    return PredictionAccuracy(
        device_id=device, window_hours=hours, total_predictions=int(r["tong_du_bao"] or 0),
        evaluated=evaluated, mae=rounded("mae"), rmse=rounded("rmse"),
        max_error=rounded("sai_so_lon_nhat"), bias=rounded("do_lech_he_thong"),
        trend_accuracy_percent=ratio, note=note)


@router.post("/alerts/test", response_model=AlertTestResponse, dependencies=[Depends(require_write_access)], tags=["Cảnh báo"],
