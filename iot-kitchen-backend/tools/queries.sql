...
-- ---------------------------------------------------------------------
-- 8. MÔ HÌNH AI - ĐỘ CHÍNH XÁC TRÊN DỮ LIỆU THẬT
--    Giống hệt phép tính của endpoint GET /prediction/accuracy.
--    Đây là con số nên đưa vào báo cáo, khác với MAE lúc train (đo trên dữ liệu mô phỏng).
-- ---------------------------------------------------------------------
WITH doi_chieu AS (
    SELECT p.time,
           p.predicted_pollution_percent AS du_bao,
           (SELECT avg(t.pollution_percent) FROM telemetry t
            WHERE t.device_id = p.device_id
              AND t.time BETWEEN p.target_time - interval '15 seconds'
                             AND p.target_time + interval '15 seconds') AS thuc_te
    FROM ai_predictions p
    WHERE p.time > now() - interval '24 hours' AND p.target_time < now()
)
SELECT count(*)                                          AS so_mau,
       round(avg(abs(du_bao - thuc_te))::numeric, 2)     AS mae_phan_tram,
       round(sqrt(avg((du_bao - thuc_te)^2))::numeric, 2) AS rmse_phan_tram,
       round(max(abs(du_bao - thuc_te))::numeric, 2)     AS sai_so_lon_nhat,
       round(avg(du_bao - thuc_te)::numeric, 2)          AS do_lech_he_thong
FROM doi_chieu
WHERE thuc_te IS NOT NULL;

-- Thời gian suy luận của mô hình (số liệu hiệu năng cho báo cáo)
SELECT count(*)                                  AS so_lan_suy_luan,
       round(avg(inference_ms)::numeric, 2)      AS trung_binh_ms,
       round(max(inference_ms)::numeric, 2)      AS lon_nhat_ms,
       count(DISTINCT model_version)             AS so_phien_ban_mo_hinh
FROM ai_predictions
WHERE time > now() - interval '24 hours';

-- Phân bố xu hướng dự báo
SELECT trend, count(*) AS so_lan,
       round(avg(predicted_pollution_percent)::numeric, 1) AS du_bao_tb
FROM ai_predictions
WHERE time > now() - interval '24 hours'
GROUP BY trend ORDER BY so_lan DESC;
