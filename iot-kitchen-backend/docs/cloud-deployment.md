...
Cấu hình khuyến nghị: 2 GB RAM, Ubuntu 24.04.

Máy 1 GB (Google Cloud e2-micro, AWS t2.micro) **vẫn chạy được đủ 5 container**, kể cả AI. Bộ nhớ
đo thực tế: dịch vụ AI 181 MB, backend 51 MB, PostgreSQL 100 MB, Mosquitto 10 MB, Caddy 20 MB,
tổng khoảng 330 MB. Chỉ cần thêm lớp cấu hình `deploy/docker-compose.lowmem.yml` và tạo swap,
xem mục 8.
...
## 7. Chạy trên máy 1 GB mà vẫn giữ container AI

Vấn đề không nằm ở lúc chạy mà ở **lúc build**. Cài scikit-learn, pandas, numpy và scipy trên máy
1 nhân 1 GB sẽ ngốn bộ nhớ đỉnh điểm 400 đến 600 MB, dễ bị hệ điều hành kill giữa chừng.

Ba cách, chọn một:

**Cách 1: tạo swap rồi build bình thường.** Đơn giản nhất, chỉ chậm.
...
**Cách 2: không build trên máy chủ.** Build trên máy Windows nhiều RAM rồi đẩy image lên Docker Hub,
máy chủ chỉ việc tải về. Cách này bỏ hẳn được đỉnh bộ nhớ.

```powershell
docker login
docker build -t <tài-khoản>/kitchen-ai:1.0 ./ai
docker build -t <tài-khoản>/kitchen-backend:1.0 ./backend
docker push <tài-khoản>/kitchen-ai:1.0
docker push <tài-khoản>/kitchen-backend:1.0
```

Trên máy chủ, sửa `docker-compose.yml` đổi `build: ./ai` thành `image: <tài-khoản>/kitchen-ai:1.0`,
tương tự cho backend, rồi `docker compose pull` và `up -d`.

**Cách 3: dùng khoản 300 USD tặng kèm.** Tài khoản Google Cloud mới được 300 USD dùng trong 90 ngày.
Máy e2-small 2 GB tốn khoảng 13 USD một tháng, tức là chạy trọn đồ án vẫn không tốn đồng nào, và
không phải tối ưu gì cả. Hết 90 ngày thì hạ xuống e2-micro miễn phí vĩnh viễn.

Khi chạy, thêm lớp cấu hình tiết kiệm bộ nhớ:

```bash
docker compose -f docker-compose.yml \
               -f deploy/docker-compose.prod.yml \
               -f deploy/docker-compose.lowmem.yml up -d --build
```

Lớp này hạ `shared_buffers` của PostgreSQL từ 128 MB xuống 64 MB và giảm số kết nối tối đa từ 100
xuống 20, tiết kiệm khoảng 40 MB. Nó cũng đặt giới hạn bộ nhớ cho từng container, nên nếu có rò rỉ
thì Docker giết đúng container đó rồi khởi động lại, thay vì treo cả máy chủ.
