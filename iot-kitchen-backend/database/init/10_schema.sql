...
-- =====================================================================
--  BẢNG 4: ai_predictions - lưu lại từng lần dự báo của mô hình AI (TV5)
--  Lưu để làm 2 việc:
--    1. Dashboard vẽ đường dự báo chồng lên đường thực đo.
--    2. ĐO ĐỘ CHÍNH XÁC TRÊN DỮ LIỆU THẬT: 15 phút sau, đối chiếu giá trị đã dự báo
--       với giá trị đo được để tính MAE/RMSE. Chỉ số MAE lúc train chỉ đúng trên dữ
--       liệu mô phỏng; con số đo ở đây mới là con số đưa vào báo cáo.
-- =====================================================================
CREATE TABLE IF NOT EXISTS ai_predictions (
    time                        TIMESTAMPTZ      NOT NULL,  -- thời điểm suy luận
    device_id                   TEXT             NOT NULL,
    ...
    current_pollution_percent   DOUBLE PRECISION,           -- giá trị lúc suy luận, để tính độ lệch
    trend                       TEXT             NOT NULL CHECK (trend IN ('RISING', 'FALLING', 'STABLE')),
    model_version               TEXT,                       -- đổi model là biết ngay số liệu thuộc bản nào
    inference_ms                DOUBLE PRECISION            -- thời gian suy luận, số liệu cho báo cáo
);
