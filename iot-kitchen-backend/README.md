...
```
 ...
 └─────────────────┘   lệnh điều    └───────────┬──────────────┘              └──────┬───────┘
                        khiển                   │ ingest                             │
                                     ...
                                     │  FastAPI Backend     │ ◄───────────────────────┘
                                     │  - ghi CSDL          │      ┌──────────────────┐
                                     │  - REST API          │ ◄──► │ Dịch vụ AI 9100  │
                                     │  - cảnh báo Telegram │──┐   │ (TV5, Tuần 2)    │
                                     └──────────┬───────────┘  │   └──────────────────┘
                                                │              └──► 📱 Telegram
                                     ┌──────────▼───────────┐
                                     │ PostgreSQL 16 +      │
...
├── ai/                         # Tuần 2: dịch vụ suy luận, bọc mô hình của TV5
│   ├── app/main.py             # HTTP API: /health, /predict, /reload
│   ├── tv5/                    # train_ai.py + ai_service.py của TV5, giữ nguyên
│   └── models/                 # đặt pollution_model.pkl vào đây
├── database/
...
├── deploy/
│   ├── vm-setup.sh             # chạy 1 lần trên máy chủ: swap, Docker, tường lửa, lệnh tắt
│   ├── backup.sh               # sao lưu CSDL, đặt lịch bằng crontab
│   ├── docker-compose.prod.yml # lớp cho máy chủ: thêm Caddy, restart always
│   ├── docker-compose.lowmem.yml # lớp cho máy 1 GB RAM: hạ shared_buffers, giới hạn bộ nhớ
│   ├── Caddyfile               # https tự động, gom API + WebSocket + Dashboard 1 tên miền
│   └── railway/                # Dockerfile riêng cho Railway (không chạy được compose)
├── mosquitto/config/
...
| `GET /prediction` | Dự báo ô nhiễm 15 phút tới | Mô hình của TV5 chạy trong container riêng; chưa dự báo được thì trường `detail` nói rõ lý do |
| `GET /prediction/accuracy` | Đo độ chính xác mô hình | MAE/RMSE trên **dữ liệu thật**, khác với MAE lúc train vốn đo trên dữ liệu mô phỏng |
...
### 5.5 Bảo vệ endpoint ghi khi chạy public

Trong LAN thì để `API_KEY` trống, gọi API như bình thường. Khi mở ra Internet thì bắt buộc đặt khoá,
nếu không ai biết địa chỉ cũng bật tắt được quạt. Cách làm và cách mở tunnel nằm trong
[`docs/public-deployment.md`](docs/public-deployment.md).

Bốn endpoint ghi (`/actuator`, `/mode`, `/config`, `/alerts/test`) yêu cầu header `X-API-Key`.
...
### 5.6 Kiểm chứng từ trình duyệt thật

Mở `tools/dashboard-demo.html` bằng Chrome. Trang này nối cả hai đường cùng lúc: WebSocket cổng
9001 cho dữ liệu tức thời và REST cổng 8000 cho lịch sử với điều khiển. Bấm được nút BẬT QUẠT và
thấy số liệu nhảy mỗi 2 giây tức là backend đã sẵn sàng cho TV4.

Đây cũng là bản mẫu để TV4 chép phần JavaScript. Chi tiết ở
[`docs/frontend-integration.md`](docs/frontend-integration.md).

---

### 5.8 Kiểm tra lớp 5 - Dự báo AI (Tuần 2)

Cần file `ai/models/pollution_model.pkl` do TV5 gửi. Chép vào thư mục `ai/models/` rồi nạp nóng,
không phải khởi động lại container:

```powershell
curl.exe -X POST http://localhost:9100/reload
```

Chưa có file thì hệ thống vẫn chạy bình thường, chỉ là `/prediction` báo rõ đang thiếu mô hình.
Cách tự train trên dữ liệu thật của hệ thống nằm ở mục 5.3 của
[`docs/ai-integration.md`](docs/ai-integration.md).

Kiểm tra theo thứ tự:

```powershell
curl.exe http://localhost:9100/health                                  # mô hình nạp chưa
curl.exe http://localhost:8000/api/v1/kitchen/prediction               # dự báo mới nhất
docker compose exec mosquitto mosquitto_sub -h localhost -u backend_svc -P 'Backend2026aA' -t 'iot/kitchen/+/ai_prediction' -v
```

Cần khoảng 210 giây dữ liệu liên tục mới có dự báo đầu tiên. Trong lúc chờ, `GET /prediction` sẽ
ghi rõ còn thiếu bao nhiêu khung dữ liệu.

Sau khi hệ thống chạy được vài giờ thì đo độ chính xác thật:

```powershell
curl.exe "http://localhost:8000/api/v1/kitchen/prediction/accuracy?hours=24"
```

Chi tiết kiến trúc và những điểm cần lưu ý về mô hình nằm trong
[`docs/ai-integration.md`](docs/ai-integration.md).
...
| `/prediction` trả `detail: chưa nạp được mô hình` | Thiếu file `ai/models/pollution_model.pkl` | Train trong container theo mục 5.7, rồi `curl.exe -X POST http://localhost:9100/reload` |
| `table "ai_predictions" does not exist` | CSDL tạo từ bản schema cũ | Chạy `database/migrations/2026-09-16_add_ai_predictions.sql` |
...
| Tích hợp AI | Suy luận 4,9-9,7 ms, publish đúng chu kỳ, lưu đủ `target_time` và `model_version` |
